...
import { QaseCoreReporter, TestResult } from '../../../src';
...
describe('end', () => {
  ...
  it('should support sending results without completing run', async () => {
    ...
    reporter.addTestResult(
      { title: 'test', status: ResultCreateStatusEnum.PASSED },
  ...
  it('should log error if there is an issue completing run', async () => {
    ...
    reporter.addTestResult(
      { title: 'incomplete run test', status: ResultCreateStatusEnum.PASSED },
      ResultCreateStatusEnum.PASSED,
    );
  ...
  it('should add test result with attachments', async () => {
    ...
    const attachmentPath =
      process.cwd() + '/test/__artifacts__/screenshots/screenshot.png';
    await reporter.start();
    reporter.addTestResult(testResult, ResultCreateStatusEnum.PASSED, [
      { path: attachmentPath },
    ]);
    await reporter.end({ spawn: false });
