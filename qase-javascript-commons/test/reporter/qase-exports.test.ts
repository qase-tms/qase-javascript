...
import { qase, qaseTitle, qaseParam } from '../../src';
...
describe('qase exports', () => {
  ...
  describe('qaseParam', () => {
    it('should return test title with param format - object, caseId array', () => {
      ...
      const newTest = qaseParam(
        [1, 2],
        [0, { a: 'a', b: 'b', expected: 'ab' }],
        test.title,
      );
      expect(newTest).toBe(
        'test (Qase ID: 1,2) (Qase Dataset: #0 (a: a, b: b, expected: ab))',
      );
    });
    ...
    it('should return test title with param format - string, no caseId', () => {
      ...
      expect(newTest).toBe(
        'test (Qase Dataset: #0 (a: a, b: b, expected: ab))',
      );
    });
    ...
    it('should return test title with param format - string, caseId string|number', () => {
      ...
      expect(newTest).toBe(
        'test (Qase ID: 1) (Qase Dataset: #0 (a: a, b: b, expected: ab))',
      );
    });
  });
