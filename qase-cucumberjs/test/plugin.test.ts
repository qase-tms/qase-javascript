      ...
      testData
        ...
        .forEach((eTestData) => {
          it(`should set defect=${eTestData.defectValue} when status=${eTestData.status}`, () => {
            expect(qReporter['results'][eTestData.caseId].defect).toBe(
              eTestData.defectValue,
            );
          });
        });
    });
