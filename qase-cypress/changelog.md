...
- Added browser-side network profiling support via Cypress event bridge.
- Network requests captured in the browser are automatically converted to REQUEST-type steps in Qase TestOps.
- Added profiler configuration propagation from `qase.config.json` to Cypress environment.
