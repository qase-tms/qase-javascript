...
/**
 * Resolve the user-provided reporter options from a Cypress `reporterOptions`
 * payload. Cypress' `reporterOptions` can arrive as:
 *   - `null` / `undefined` — no reporter configured in cypress.config
 *   - `{ cypressQaseReporterReporterOptions: {...} }` — generated by
 *     `cypress-multi-reporters` from the package name
 *   - `{ testops: {...}, mode: 'testops', ... }` — passed directly when the
 *     user sets `reporter: 'cypress-qase-reporter'` without the multi-reporter wrapper
 * Returning `null` lets `composeOptions` fall back entirely to the file/env config.
 */
function resolveUserOptions(reporterOptions: unknown): unknown {
  if (reporterOptions == null || typeof reporterOptions !== 'object') {
    return null;
  }
  const wrapped = (reporterOptions as Record<string, unknown>)['cypressQaseReporterReporterOptions'];
...
async function beforeRunHook(options: PluginConfigOptions) {
  ...
  const userOptions = resolveUserOptions(options?.reporterOptions);
  // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
  const { ...composedOptions } = composeOptions(userOptions, config);
