...
class CypressQaseReporter extends reporters.Base {
  ...
  public constructor(runner: Runner, options: MochaOptions) {
    ...
    this.options.rootSuiteTitle =
      CypressQaseReporter.getEnv(Envs.rootSuiteTitle) ||
      options.reporterOptions.rootSuiteTitle ||
      "";
    this.options.projectCode =
      options.reporterOptions.projectCode ||
      CypressQaseReporter.getEnv(Envs.projectCode);
  ...
  private static getSuitePath(suite): string {
    if (suite.parent) {
      const parentSuite = String(
        CypressQaseReporter.getSuitePath(suite.parent)
      );
      if (parentSuite) {
  ...
  private async createRun(
    ...
    try {
      ...
      const planId =
        Number(CypressQaseReporter.getEnv(Envs.planId)) || this.options.planId;
    ...
    if (caseIds.length === 0) {
      caseObject.case = {
        ...
        suite_title: this.options.rootSuiteTitle
          ? `${this.options.rootSuiteTitle}\t${CypressQaseReporter.getSuitePath(
              test.parent
            )}`
          : CypressQaseReporter.getSuitePath(test.parent),
      };
      this.resultsForPublishing.push(caseObject);
      this.log(chalk`{gray Result prepared for publish: ${test.title} }`);
    } else {
