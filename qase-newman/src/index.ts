...
import { ResultCreateStatusEnum } from "qaseio/dist/src/model";
...
class NewmanQaseReporter {
  ...
  public constructor(
    ...
    this.runIdP = this.checkProject(this.options.projectCode, (prjExists) => {
      if (!prjExists) {
      ...
      if (providedRunId) {
        return this.checkRun(providedRunId, (runExists: boolean) => {
          if (runExists) {
            this.log(
              chalk`{green Using run ${providedRunId} to publish test results}`
            );
            return Promise.resolve(providedRunId);
          } else {
            return Promise.reject(
              chalk`{red Run ${providedRunId} does not exist}`
            );
          }
        });
      } else {
  ...
  private createHeaders() {
    ...
    const xClientHeader = `jest=${jestVersion as string}; qase-jest=${
      jestCaseReporterVersion as string
  ...
  private checkProject(
    projectCode: string,
    cb: (exists: boolean) => Promise<RunId>
  ) {
  ...
  private createRun(
    ...
    return this.api.runs
      .create(
        this.options.projectCode,
        new RunCreate(name || `Automated run ${new Date().toISOString()}`, [], {
      ...
      test.ids.forEach((caseId) => {
        ...
        this.api.results
          .create(
            this.options.projectCode,
            runId,
            new ResultCreate(parseInt(caseId, 10), test.result, {
              // eslint-disable-next-line camelcase
          ...
          .then((res) => {
            ...
            this.log(
              chalk`{gray Result published: ${test.name} case ${caseId}}`
            );
          })
          .catch((err) => {
